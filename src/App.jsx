...
const STAT_NAMES = { STR: "Strength", MND: "Mind", DIS: "Discipline", VIT: "Vitality", CRE: "Creativity" };

function generateDailyNote(data, todayKey) {
  const streak    = calcStreak(data.dailyLog, data.shieldedDays);
...
function generateHunterProfile(data) {
  const streak      = calcStreak(data.dailyLog, data.shieldedDays);
  const need        = xpNeed(data.level);
  const totalXp     = Object.values(data.dailyLog).reduce((a, b) => a + b.xp, 0);
  const totalQuests = Object.values(data.dailyLog).reduce((a, b) => a + b.quests, 0);
...
function generateQuestArchive(data) {
  const lines = [
    `---`,
...
function exportToObsidian(content, filePath, vaultName) {
...
function StatCard({ statKey, value }) {
  ...
  return (
    <motion.div whileHover={{ y: -3, scale: 1.04 }} transition={{ type: "spring", stiffness: 280, damping: 18 }} className={`stat-card sc-${m.color}`}>
      ...
      <span className="stat-val" title={value} style={{ fontSize: STAT_VAL_SIZE[display.length] || "0.82rem" }}>{display}</span>
